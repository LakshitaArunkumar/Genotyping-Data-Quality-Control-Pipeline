---
title: "Genotyping-Data-Quality-Control-Pipeline"
output:
  html_document:
    code_folding: show
    toc: true
...
# Your code goes here
#Question1: What is the difference between the scanID and subjectID variables? Why are they both provided?
scan_annodf <- scanAnnot
meta_scan <- getMetadata(scan_annodf)
meta_scan
blData
#Code for Question1:
scan_annodf <- scanAnnot
...
#Answer1: From the meta_scan dataframe we get to know that scanID is a unique set of ID for each scan and corresponds one subjectID. subjectID is subject identifier and it can have many scanID. Both the varaiables are required to understand which scan/sample belongs to which subject. Help us differentiate between subject level data and sample level data (which we used in project1 of 2071.)

#Question2: How many families are represented, and how many samples are from each family

#Code for Question2:
scan_annodf <- pData(scan_annodf)
table(duplicated(scan_annodf$family))
unique_family <- unique(scan_annodf$family)
cat("Families which are unique:", unique_family, "\nNumber of unique families:", length(unique_family), "\n")

num_family<- scan_annodf %>%
  group_by(family) %>%
  summarize(sample_count = n())
num_family

#Answer2: From duplicated we can see that there are 13 families, the variable unique_family has the list of families which have the unique number. The dataframe num_family has number of samples for each family. 

#Question3: How many samples are of each sex?
...
#Answer3: There are 35 females and 42 males

#Question4: How many samples are from each population group?

#Code for Question4: 
num_race<- scan_annodf %>%
  group_by(race) %>%
  summarize(sample_count = n())
num_race

#Answer4: There are 49 samples of race CEU and 28 samples of race YRI 

#Question5: How many samples are there? How many people did they come from?

#Code for Question5:
num_samples <- unique(scan_annodf$scanID)
cat("/nNumber of unique samples:", length(num_samples))

num_subject<- scan_annodf %>%
  group_by(subjectID) %>%
  summarize(sample_count = n())
head(num_subject)
tail(num_subject)

num_unique_sub <- unique(scan_annodf$subjectID)
cat("/nNumber of unique subjects:", length(num_unique_sub))

#Answer5: There are 77 unique samples and 43 unique subjects. In dataframe num_subject we can see that how many samples were there was each subjectID.  

#Question6: How many SNPs are there?

#Code for Question6:
snp_data <- snpAnnot
snp_data <- pData(snp_data)
meta_snp <- getMetadata(snpAnnot)
num_snp <- length(unique(snp_data$snpID))
cat("/nNumber of unique SNPs:" ,num_snp)
table(duplicated(snp_data$snpID))

#Answer6: There are unique 3300 SNPs

#Question7: Which chromosomes are represented? How many SNPs are on each?
...
#Answer7: The number of SNPs across chromosomes is as follows: Chromosome 21 has 1000 SNPs, Chromosome 22 has 1000 SNPs, Chromosome 23 has 1000 SNPs, Chromosome 24 has 100 SNPs, Chromosome 25 has 100 SNPs, and Chromosome 26 has 100 SNPs.
...
#   <Your code goes here>
scan_exlcude <- scanAnnot$scanID[scanAnnot$missing.e1 > 0.05]
cat("Number of scanIDs needs to be excluded when missing.e1 > 0.05:", length(scan_exlcude), "\n")

miss_2 <- missingGenotypeBySnpSex(genoData, scan.exclude = scan_exlcude)
allequal(snpAnnot$snpID, as.numeric(names(miss_2$missing.fraction)))

snpAnnot$missing.n2 <- miss_2$missing.fraction

varMetadata(snpAnnot)["missing.n2", "labelDescription"] <-
  paste("fraction of genotype calls missing over all samples with missing.e1 < 0.05 except that females are excluded for Y chr SNPs")


tail(getMetadata(snpAnnot))
```
...
#   <Your code goes here>
snp_exlcude <- snpAnnot$snpID[snpAnnot$missing.n2 >= 0.05]
cat("Number of snpsIDs needs to be excluded when missing.n2 >= 0.05:", length(snp_exlcude), "\n")
miss_3 <- missingGenotypeByScanChrom(genoData, snp.exclude = snp_exlcude)
allequal(names(miss_3$missing.fraction), scanAnnot$scanID)

scanAnnot$missing.e2 <- miss_3$missing.fraction

varMetadata(scanAnnot)["missing.e2", "labelDescription"] <-
  paste("fraction of genotype calls missing over all snps with missing.n2 < 0.05 except that Y chr SNPs are excluded for females")


tail(getMetadata(scanAnnot))
```
...
#   <Your code goes here>
#Question1: (First, make sure you've filled in the calculations of `missing.n2` and `missing.e2` in the chunks above.)

head(pData(snpAnnot))
head(pData(scanAnnot))

#Question2: How many SNPs and samples were genotyped?

#Code for Question2:
scan_annodf<-pData(scanAnnot)
cat("Number of samples:", length(unique(scan_annodf$scanID)), "\n")
snp_data<-pData(snpAnnot)
cat("Number of SNPs:", length(unique(snp_data$snpID)), "\n")

#Answer2: Number of samples: 77  and Number of SNPs: 3300

#Question3: Briefly describe the purpose of doing this step in two rounds (why do we calculate both missingnesses twice?)

#Answer3: The first round of data cleaning removes samples and SNPs with a high proportion of missingness. The second round removes any remaining samples and SNPs with >=5% missing data. This ensures the dataset is clean and free of outliers, allowing for reliable downstream analysis, such as the analysis of sex and alleles (allelic imbalance using BAF and LRR plots).

#Question4: What were the median, mean, and maximum per-sample missingness (use `missing.e2`)? Make a histogram of `missing.e2`

#Code for Question4:
summary(scanAnnot$missing.e2)

#Code for histogram:
hist(scanAnnot$missing.e2,
  xlab = "Sample over all SNPs with missing.n2 =< 0.05",
  main = "Histogram of Sample Missing Call Rate for All SNPs - Round 2")

#Answer4:From the summary we see that the medium = 0.0006544, mean = 0.0018289, maximum= 0.0242147 

#Question5: How many samples had no missing calls at all (use `missing.e2`)?
...
#Answer5: There are 6 samples which have no missing calls at all

#Question6:  Are the samples generally "good"? How many should be excluded from downstream analyses?

#Code for Question6: 

#Answer6: From the histogram, the samples are "good" because there are no samples that have a missingness rate >=0.05. If such samples were present, they would need to be excluded from downstream analyses to ensure data quality. 

#Question7: How many SNPs had a 100% missing call rate (use `missing.n1`)?

#Code for Question7:
snp_data %>%
  group_by(missing.n1) %>%
  filter(missing.n1 == 1) %>%
  summarize(snps_count = n())

#Answer7: There are 151 snps with 100% missing call rate

#Question8: What are the median, mean, and maximum per-SNP missingnesses (use `missing.n2`)? How many SNPs are missing no calls at all?

#Code for Question8:
summary(snpAnnot$missing.n2)

#Answer8:From the summary we see that the medium = 0.00000, mean = 0.04898, maximum= 1.00000

#Question9: Make a histogram of `missing.n2`

#Code for Question9:
hist(snpAnnot$missing.n2,
  ylim = c(0, 100),
  xlab = "SNP over all samples whose missing.e1 is less than 0.05",
  main = "Missing Call Rate for All Probes - Round2")

#Answer9: The histogram shows that there are many snps with high missingness rate. 

#Question10:  How many SNPs are filtered out and how many remain if you retain only SNPs with `missing.n2` < 0.05?

#Code for Question10:
snp_data %>%
  filter(missing.n2 >= 0.05) %>%
  nrow(.)
snp_data_retain <- snp_data %>%
  filter(missing.n2 < 0.05)
nrow(snp_data_retain)

#Answer10: The number of SNPs retained are 3095 and the number of filtered out SNPs are 205.

#Question11: Make a histogram showing `missing.n2` for the remaining SNPs

#Code for Question11: 
hist(snp_data_retain$missing.n2,
  ylim = c(0, 100),
  xlab = "SNP missing call rate < 0.05",
  main = "Missing Call Rate After Filtering")
```
...
#   <Your code goes here>
#Question1:Describe any anomalies you see in the 4 plots above.

#Answer1: i) From the plots Mean X vs Mean Y Chromosome Intensity; Mean X Chromosome Intensity vs Mean X Chromosome Heterozygosity; Mean X Chromosome Heterozygosity vs Mean Y Chromosome Intensity the 2 samples which were reported females they are clustered within male data points. This suggests potential misclassification, possibly due to data entry errors, sample mix-ups(not likely as we donot see any male samples in female clouds), or karyotypic variations. ii) From the plot Mean Autosomal Heterozygosity vs Mean X Chromosome Heterozygosity we can get to know there are outliers which could mean sex chromosome aneuploidy.

#Question2: Figure out the scanIDs of any scans that look like outliers or possible instances of mislabeled sex. Include these IDs in your report and describe why they appear to be mislabeled.

#Code for Question2:

scan_annodf <- pData(scanAnnot)

#Based on X and Y chromosome intensity
Male_scanID_inten<-  rownames(mninten[mninten[, "X"] < 1 & mninten[, "Y"] > 0.8, , drop = FALSE])
misclassification_outlier_scan1 <- scan_annodf %>%
  filter(scanID %in% Male_scanID_inten & sex == 'F') %>%
  select(scanID, sex)
kable(misclassification_outlier_scan1, caption = "scanID of mislabeled sex (X and Y chr intensity, expected male)")

#Based on heterozyogosity

misclassification_outlier_scan2 <- scan_annodf %>%
  filter(het.X==0 & sex == "F") %>%
    select(scanID, sex, het.X)
kable(misclassification_outlier_scan2, caption = "scanID of the mislabeled sex using heterozyogosity")

#Answer2:The two misclassification and outlier scanID are 325 and 326

#Question3: For the purposes of this assignment we will ASSUME that some scans have just had their sexes mislabeled. (In "real life", we would not just assume this, and we might instead flag these scans for removal from some downstream steps/analyses.) 

#Manually change the sex annotation for these scans in scanAnnot to "remove" the discrepancy. (Manually changing your data is risky and not generally recommended. Be careful!). In your report, state exactly what changes you made. Add a new column to scanAnnot indicating this, too (add appropriate metadata). Does anything in the pedigree structure make you more confident that this "fix" is correct? (Are these samples labeled as mothers or fathers?)

#Code for Question3:
scan_annodf$sex_updated <- ifelse(scan_annodf$scanID %in% c(325, 326), "M", scan_annodf$sex)
show_changed_sex <- scan_annodf %>%
  filter(scanID == 325 | scanID == 326) %>%
  select(scanID, sex, sex_updated, subjectID)
kable(show_changed_sex, caption = "Sex for the ids have been changed")

parent_check <- scan_annodf %>%
  filter(father == 200094287 | mother == 200094287)
kable(parent_check, caption = "Parent check")

pData(scanAnnot) <- scan_annodf
head(pData(scanAnnot))

scan_meta <- getMetadata(scanAnnot)
scan_meta["sex_updated", "labelDescription"] <- "The mislabeled sex of the sample has been corrected" 
varMetadata(scanAnnot) <- scan_meta
getMetadata(scanAnnot)

#Answer3: The sex of the scanID 325 and 326 have been changed to M(male). In the table "Sex for the ids have been changed" we can see that. When checked if the scanID's sujectID which is 200094287 is a parent (mother / father) which could disrupt the pedigree, since the subjectID is father of 200074814 so after discussing with the group we can come to a conclusion that the sex of the scanID 325 and 326 has been mislabeled, and by changing the sex it will not disrupt the pedigree of the family

#Question4: Remake the 4 plots above, using the "corrected" data (nothing should appear mislabeled now for this dataset).

#Code for Question4:
xcol <- rep(NA, nrow(scanAnnot))
xcol[scanAnnot$sex_updated == "M"] <- "blue"
xcol[scanAnnot$sex_updated == "F"] <- "red"
...
x1 <- mninten[, "X"]
y1 <- mninten[, "Y"]
main1 <- "MeanX Chr vs MeanY Chr Intensity (Post-Sex Correction)"

x2 <- mninten[, "X"]
y2 <- scanAnnot$het.X
main2 <- "MeanX Chr Intensity vs MeanX Chr Heterozygosity \n(Post-Sex Correction)"


y3 <- mninten[, "Y"]
x3 <- scanAnnot$het.X
main3 <- "MeanX Chr Heterozygosity vs MeanY Chr Intensity \n(Post-Sex Correction)"


x4 <- scanAnnot$het.A[scanAnnot$sex_updated == "F"]
y4 <- scanAnnot$het.X[scanAnnot$sex_updated == "F"]
main4 <- "Mean Autosomal Heterozygosity vs MeanX Chr Heterozygosity \n(Post-Sex Correction)"
...
#Answer4: The four plots plotted does not have any anomalies like the previous set of plots.

#Question5: How might you follow up on this "in real life"?

#Answer5: I would verify the reported sex against clinical records and check for data entry errors. Next, I’d analyze X and Y chromosome intensities to identify potential aneuploidy (e.g., Turner or Klinefelter syndrome). Reviewing pedigree data would ensure changes align with inheritance patterns. If discrepancies remain, I’d consult previous genetic reports and discuss with the team whether to correct or exclude the samples while documenting all changes. 
```
...
#  <Your code goes here>
#Question1: Describe the purpose of this step.

#Answer1: This step is performed to assess copy number variations across the chromosome, helping to identify potential duplications or deletions that may be relevant to genomic analysis.

#Question2: Show an example of a "normal" pair of BAF/LRR plots (choose any chromosome/scanID you want).

#Code for question2:
chromIntensityPlot(blData, scan.ids = 283, chrom.ids = 22)

#Answer2: The BAF plot has 3 bands and the LRR plot has the points at 0 so the data is a disomy which is normal

#Question3: How would you detect trisomy 21 (i.e., describe briefly in words what the BAF/LRR plot would look like).

#Answer3: In the case of trisomy 21, the BAF plot would show four bands. Two of these bands would be at 0 and 1. The other two bands, at 0.33 and 0.67, represent the extra, third copy of the chromosome. On the LRR plot, you'd see a noticeable increase above 0, which suggests a gain of DNA.

#Question4: Suppose an individual had a balanced reciprocal translocation involving chromosome 2 and 4. What would you expect that individual's BAF/LRR plots would look like for those chromosomes?

#Answer4: The BAF and LRR plots would be same as normal BAF plot with 3 bands and normal LRR plot with points at 0 which shows no loss or gain of DNA.  

#Question5: Suppose chromosome 22 in sample 286 was the only chromosome flagged for a possible anomaly in this dataset. Looking at the plot, do you notice anything strange? Can you suggest what kind of anomaly might be showing up? (There isn't any single answer that's certainly right! The purpose of this question is to check that you understand what the BAF and LRR plots "mean").

#Answer5: One possible anomaly could be trisomy 22 where we would see 4 bands in BAF plot and LRR plot will have an increase (shows gain of DNA) above 0. Second, possible anomaly could be loss of heterozygosity where the BAF plot will have only 2 bands one at 0 and one at 1 and LRR plot will have an decrease (shows loss of DNA).Third, possible anomaly could be because of the the outliers points spread across the BAF and LRR plots in a certain section which makes them look like they have an extra band.  

#Question6: Finally, suppose your pipeline has given you a list of the exact chromosomal segments showing possibly aneuploidies/anomalies in each sample. What would you recommend doing with the genotypes in those segments? (You're not being asked to implement this here - just say what you recommend doing.)

#Answer6: If anomalies are detected across an entire chromosome in a sample, I would recommend documenting the finding and excluding the entire chromosome (removing the genotype) from further analysis. However, if only a specific segment of the chromosome shows anomalies(deletion/duplication), I would suggest documenting that segment and excluding just that part of the chromosome (removing the genotype from this segment). If contaminated sample gets flagged then we would have to remove the samples after documenting it. These decisions should be made after a thorough discussion with the team.
```
...
snps_to_include_for_kinship <-
  snpAnnot$snpID[snpAnnot$missing.n2 < 0.05]
...
ped <-
  pData(scanAnnot)[, c("family", "subjectID", "father", "mother", "sex_updated")]
dim(ped)
...
rels <- GWASTools::pedigreePairwiseRelatedness(ped.complete)
...
```{r relatedness4, eval=T}

#   Recalculate the relatedness as above

gds_geno <- system.file("extdata", "illumina_geno.gds", package = "GWASdata")
snps_to_include_for_kinship <- snpAnnot$snpID[snpAnnot$missing.n1 < 0.05]
gdsobj <- snpgdsOpen(gds_geno)
ibdobj <- snpgdsIBDKING(gdsobj, snp.id = snps_to_include_for_kinship)
snpgdsClose(gdsobj)
...
ped <- pData(scanAnnot)[, c("family", "subjectID", "father", "mother", "sex_updated")]
dim(ped)
names(ped) <- c("family", "individ", "father", "mother", "sex")
head(ped)
chk <- pedigreeCheck(ped)
...
ped.complete[ped.complete$family == "58",  c("father", "mother")] <- c("1111", "2222")
ped.complete[ped.complete$family == "58-2", c("father", "mother")] <- c("3333", "4444")
...
#Question1: Summarize the pedigree structures you observe in the data.
#Code for Question1:
ibd$obs.rel <- ibdAssignRelatednessKing(ibd$IBS0, ibd$kinship)
table(ibd$obs.rel, useNA = "ifany")
table(ibd$exp.rel, ibd$obs.rel)

#Answer1: From the kingship plot we see that the most of the unrelated data is below 0 and close to 0 and there are no  inconsistencies between the expected and observed genetic relatedness statistics; the parent-offspring pairs are in it's correct place with expected kingship value = 0.25 and fraction of IBS = 0; the duplicates pairs has an expected kingship value = 0.5 are in it's correct place. The function ibdAssignRelatednessKing is used to call observed relationship based on IBS0 and kinship values for each pair. From which we get to know that there are 2802 unrelated (1401 pairs), 34 duplicates (17 pairs) and 90 parent-offspring (45 pairs). 

#Question2: Describe what discrepancies you observe in the initial kinship vs IBS0 plot.

#Answer2: 16 pairs were reported as unrelated in the pedigree but were found to be genetically related, with kingship coefficients around 0.25, indicating parent-offspring relationships. 17 pairs that were supposedly parent-offspring pairs appeared genetically unrelated. This discrepancy resulted in blue clusters appearing in the unrelated clustering area, where the kinship coefficient is 0. 

#Question3: Explain exactly the kinds of swaps/discrepancies that were found, which IDs were involved, and exactly how you "fixed" them.

#Answer3: One of the discrepancies where found in families 58, 1334 and 1344. 
#In Family 58, there is an issue with scan IDs 355 and 356. Although 356 is listed as the father of 355, they are actually unrelated. The solution is to change the family ID of the father and update the father column to 0 for the child's scan ID to reflect that they are unrelated.

#In Families 1334 and 1344, the issue was that the children (296, 297, 298, 299) in the 2 families were swapped. The solution is for this is to swap the familyID, father and mother of the children accordingly. Scan IDs 296 and 297 were reassigned to family 1334 with parents 200118596 (father) and 200019634 (mother). Scan IDs 298 and 299 were reassigned to family 1344 with parents 200116780 (father) and 200005043 (mother). 

#Question4: How would you recommend following up on these discrepancies "in real life" (instead of just assuming we know what happened and manually changing the data, which is a *bad idea*)?

#Answer4: There are two approaches we can take. First, if we can resolve the discrepancy through additional checks, such as reviewing the data or going back to the field for clarification, we should keep the data and document the process. If we are unable to resolve the issue, we should add the data to an exclude list, ensuring that we note the uncertainty. It’s important to discuss such discrepancies with the team, gather input, and decide the next steps.

#Question5: Add a column to scanAnnot called "relationship.changes", which briefly summarizes these "fixes" (add metadata for the column, too).

#Code for Question5:
scan_annodf <- getAnnotation(scanAnnot) 

scan_annodf$relationship.changes <- "No changes"
for (i in 1:nrow(scan_annodf)) {
  if (scan_annodf$scanID[i] %in% c(355, 356)) {
    scan_annodf$relationship.changes[i] <- "Changed family ID and father ID since they are not related."
  } else if (scan_annodf$scanID[i] %in% c(296, 297, 298, 299)) {
    scan_annodf$relationship.changes[i] <- "Swapped family, father, and mother IDs to correct sample swap between families 1334 and 1344."
  }
}
...
changed_scanIDs <- scan_annodf %>%
  filter (relationship.changes != "No changes") %>%
  select("scanID", "relationship.changes")
kable(changed_scanIDs, caption = "ScanIds with relationship changes which were done")

varMetadata(scanAnnot)["relationship.changes", "labelDescription"] <-  "Summary of changes made to correct pedigree relationships."
...
#Question1: How many SNPs were used to calculate PCs?

#Code for Question1:
cat("Number of snps used to calculate PCAs:", length(snp.pruned), "\n")


#Question2: Make pairwise scatterplots of the first 4 PCs, using different plotting symbols for the two populations, as reported in the "race" variable in scanAnnot.

#Code for Question2:
scan_annodf <- getAnnotation(scanAnnot)
pc_df <- as.data.frame(pca$eigenvect[, 1:4])
dim(pc_df)
plot_data <- scan_annodf %>%
  filter(duplicated == "FALSE") %>%
  select("scanID", "race") %>%
  cbind(pc_df) %>%
  rename(., PCA1 = V1, PCA2=V2, PCA3=V3, PCA4=V4)
head(plot_data)
dim(plot_data)
...
num_race <- plot_data %>%
  ...
  count()
num_race
pairs(plot_data[3:6], main = "Pairwise scatter plot for first four PCAs",
      pch = 21, bg = ifelse(plot_data$race == "CEU", "red", "green"))

#Answer2: we know that there are 2 race CEU (26 samples) -> red  and YRI (17 samples) -> green

#Question3: Make a parallel coordinate plot for the first 4 plots as well, again distinguishing the plotted lines according to the "race" variable.
...
ggparcoord(plot_data,
  ...
  theme_minimal() +
  labs(title = "Parallel Coordinate Plot (Grouped by Race)- for first 4 PCAs",
       x = "Variables",
       y = "Scaled Value") +
  scale_color_manual(
    name = "Race",
    values = c("CEU" = "red", "YRI" = "green"))

#Question4: What information does the first PC appear to convey?

#Answer4: From both the plots we see that the first principal component (PCA1) captures the largest variance in the dataset, which shows there is huge separation between the two race YRI(green dots/lines) and CEU(red dots/lines) which shows the genetic difference between the population which likely reflects major ancestry-related genetic differences. 
```
...
snp_meta <- getMetadata(snpAnnot)
snp_meta["discordant", "labelDescription"] <- "The number of duplicate pairs where the SNP was called differently"
varMetadata(snpAnnot) <- snp_meta
...
#   <Your code goes here>
#Briefly explain the purpose of this step and what it means to check for discordant genotypes.
#These mismatches can happen due to errors in the genotyping process, like technical issues, sample mix-ups, or just low-quality data. By checking for discordance, we can see which SNPs are inconsistant and decide whether they should be excluded from our analysis.

#Question1: How many SNPs are discordant in at least one duplicate-pair?

#Code for Question1: 
snp_disc<- sum(dupdisc$discordance.by.snp$discordant > 0)
cat("Number of SNPs are discordant in atleast one duplicate pair:", snp_disc, "\n")

#Answer1:Number of SNPs are discordant in atleast one duplicate pair are 25 

#Question2: What's the maximum number of discordance seen for any SNP?

#Code for Question2:
max_disc <- max(dupdisc$discordance.by.snp$discordant)
cat("Maximum number of discordances:", max_disc, "\n")

#Answer2: Maximum number of discordances: 2

#Question3: What's the median discordance rate? What's the maximum discordance rate?

#Code for Question3:
summary(dupdisc$discordance.by.snp$discord.rate)

#Answer3: Median of the discordance rate is 0 and maximum discordance rate is 0.0740741 

#Question4: Decide and explicitly state the threshold, n, for filtering out SNPs based on discordance (i.e., SNPs with dupdisc$discordance.by.snp >= n will be removed) You may find it helpful to call the function duplicateDiscordanceProbability(npair=34). Read the documentation for this function.

#Code for Question4:
threshold_data <- duplicateDiscordanceProbability(npair = 34)
threshold_data 

#Answer4: Filter out SNPs with at least one discordant genotype call (n=1). This means that if a SNP shows even one mismatch between duplicate samples, should be removed from our analysis. Based on the duplicateDiscordanceProbability(npair = 34) function, which helps determine an appropriate threshold given that we have 34 duplicate sample pairs. When the number of duplicate pairs falls in this range (30–75 pairs), a threshold of n is lesser than or equal to 1 is used. This ensures that we’re keeping only the most reliable SNPs and minimizing genotyping errors in our data. 

#Question5: Of the SNPs pass that passed the missing.n2 filter (missing.n2 < 0.05), how many SNPs pass/fail the discordance threshold? 
#Code for Question5:
snp_data <- getAnnotation(snpAnnot)

threshold <- 1

num_pass <- snp_data %>%
  filter(missing.n2 < 0.05 & discordant < threshold) %>%
  nrow(.)
cat("Number of snp passed:", num_pass, "\n")

num_fail <- snp_data %>%
  filter(missing.n2 < 0.05 & discordant >= threshold) %>%
  nrow(.)
cat("Number of snp failed:", num_fail, "\n")

#Answer5: After filtering for missing.n2 < 0.05, the number of SNPs that passed is 3,074. There are 21 SNPs that failed.

#Question6: Of the SNPs with missing.n2 < 0.05, make a histogram of the discordance rate
...
pData(snpAnnot) <-
  merge(
    x = pData(snpAnnot),
    y = dupdisc$discordance.by.snp[, c("snpID", "discord.rate")],
    all.x = T,
    all.y = F
  )
...
snp_meta <- getMetadata(snpAnnot)
snp_meta["discord.rate", "labelDescription"] <- "The proportion of discordant calls out of all duplicate pairs tested"
varMetadata(snpAnnot) <- snp_meta

hist(snp_data$discord.rate[snp_data$missing.n2 < 0.05 & !is.na(snp_data$discord.rate)], 
  main = "Histogram of SNP Discordance Rates",
  ylim = c(0, 100),
  xlab = "Discordance Rate", 
  ylab = "Number of SNPs")

#Question7: Make sure you add a new column to snpAnnot called failed.dup.disc ("pass" if the SNP had >=n discordant calls, "fail" if it failed, "untested" if it was one of the high-missingness SNPs that we excluded); update the metadata

#Code for Question7:

snp_data$failed.dup.disc <- ifelse(!is.na(snp_data$discordant)&snp_data$discordant>= threshold& snp_data$missing.n2 < 0.05 , "fail", 
                                   ifelse(!is.na(snp_data$discordant) & snp_data$discordant < threshold & snp_data$missing.n2 < 0.05 , "pass", "untested"))

kable(table(snp_data$failed.dup.disc), caption="Number of SNPs based on discordance filter")
pData(snpAnnot) <- snp_data
head(pData(snpAnnot))

snp_meta <- getMetadata(snpAnnot)
snp_meta["failed.dup.disc", "labelDescription"] <- "SNP discordance filter: pass (<n), fail (>=n), untested (high missingness)."
varMetadata(snpAnnot) <- snp_meta
tail(getMetadata(snpAnnot))
```
...
#   <Your code goes here>
#Briefly explain the purpose of HWE testing
#HWE testing is used to assess the genotypes if they are under equilibrium conditions. Deviations can indicate issues such as genotyping errors, selection, population stratification.

#Question1: How do you choose what subjects to include for HWE testing? How many were used here?

#Code for Question1:
subj.incl <-unique(scan_annodf$subjectID[scan_annodf$race == "CEU" & scan_annodf$mother==0 & scan_annodf$father ==0])
length(subj.incl) 

#Answer1: Include only founder who have independent genotype. All the subject to come from same population to avoid stratification and remove duplicates. There are 17 subjectIDs who where included in the study.

#Question2: Interpret the Q-Q plots. Do any SNPs appear to be strongly out of HWE? Which SNPs? What are the lowest p-values?

#Code for Question2:
snp_follow_hwe <- unique(hwe$snpID[!is.na(hwe$pval) & hwe$pval > 1e-4])
snp_no_hwe <- unique(hwe$snpID[!is.na(hwe$pval) & hwe$pval<=1e-4])
length(snp_no_hwe)
snp_no_hwe
min(hwe$pval, na.rm = TRUE)

#Answer2: The Q-Q plot shows that most SNPs follow Hardy-Weinberg equilibrium (HWE), but SNP 1015218 stands out as deviating from HWE, which is below the threshold of 1e-4. The lowest p-value is 0.0000562

#Question3: For the autosomal SNPs, make a plot of -log10(p-value) vs MAF. Make another such plot for the X chromosome.

#Code for Question3:
#Autosomal plot
plot(hwe.0$MAF[hwe.0$chr %in% 1:22], -log10(hwe.0$pval[hwe.0$chr %in% 1:22]),
     xlab = "MAF", ylab = "-log10(p)", main = "Autosomes")

plot(hwe.0$MAF[hwe.0$chr == 23], -log10(hwe.0$pval[hwe.0$chr == 23]),
     xlab = "MAF", ylab = "-log10(p)", main = "X Chromosome")


#Question4: Add the HWE p-values to the SNP annotation (and update the metadata) What p-value will you use for filtering our SNPs with low HWE? Of the SNPs with missing.n2 < 0.05, how many will be retained/removed?
...
snp_meta <- getMetadata(snpAnnot)
snp_meta["pval", "labelDescription"] <- "Hardy-Weinberg Equilibrium p-value"
varMetadata(snpAnnot) <- snp_meta
tail(getMetadata(snpAnnot))

snp_data <- pData(snpAnnot)
num_snps_retained <- snp_data %>%
  filter(missing.n2 < 0.05 & pval > 1e-4) %>%
  nrow(.)
cat("Number of snp retained:", num_snps_retained, "\n")

num_snps_removed <- snp_data %>%
  filter(missing.n2 < 0.05 & pval <= 1e-4) %>%
  nrow(.)
cat("Number of snp removed:", num_snps_removed, "\n")

#Answer4: Retained: SNPs with p < 1e-4 -> 2455 snps . Removed: SNPs with <= 1e-4 -> 1 snps.

```

##Acknowledgment
This work was done is parallel with Ima Grad-Student. However, I take ultimate responsibility for the content of this assignment.
